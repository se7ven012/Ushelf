...
router.post('/clickpayment.json', function(req, res) {
        pool.getConnection(function(err, connection) {
            ...
            var data = {};
            var orderDetail = [];
            connection.query(userSQL.getOrder, [param.id], function(err, res) {
                if (res) {
                    // 获取用户发的聊天记录
                    for (i = 0; i < res.length; i++) {
                        var content = {};
...
router.post('/editListings.upload', function(req, res) {
    pool.getConnection(function(err, connection) {
        ...
        var param = JSON.parse(JSON.stringify(req.body));s
        var _res = res;
...
router.post('/getchatmsg.json', function (req, res) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query(userSQL.getmsg, [req.session.user.account, saytosb], function (err, res) {
            if (res) {
                console.log('history chat found')
                // 获取用户发的聊天记录
                for (i = 0; i < res.length; i++) {
                    var content = {};
                    content.fromsb = (res[i].fromsb);
                    content.time = (moment(res[i].publishTime).format('YYYY-MM-DD HH:mm:ss'));
